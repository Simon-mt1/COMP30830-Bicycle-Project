...
def getWeatherData():
    ...
    try:
        ...
        for item in data["hourly"]:
            newdata = {}
            newdata["temp"] = int(str(item["temp"]).split('.')[0])
            newdata["wind_speed"] = item["wind_speed"]
            newdata["icon"] = item["weather"][0]["icon"]
...
def map():
    if "user" in session:
        ...
        if not jcdecauxData or not weatherData:
            ...
            jcdecauxData = list(sorted(jcdecauxData, key = lambda item : item["address"]))
